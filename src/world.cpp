...
const QPixmap &getMapIcon(int opt, VarPos *vp)
{
    if (opt == D_REMOTE)
    {
        return Rpc::get()->getPixmapForRemoteMarker(vp);
    }
...
void getStructs(std::vector<VarPos> *out, const StructureConfig sconf, WorldInfo wi, int dim, int x0, int z0, int x1,
                int z1)
{
    ...
    for (int i = si0; i <= si1; i++)
    {
        for (int j = sj0; j <= sj1; j++)
        {
            ...
            if (p.x >= x0 && p.x < x1 && p.z >= z0 && p.z < z1)
            {
                ...
                {
                    int n = getFortressPieces(pieces, sizeof(pieces) / sizeof(pieces[0]), wi.mc, wi.seed, p.x >> 4,
                                              p.z >> 4);
                    vp.pieces.assign(pieces, pieces + n);
                }
                else if (g.mc >= MC_1_18)
                {
                    if (g_extgen.estimateTerrain && !isViableStructureTerrain(sconf.structType, &g, p.x, p.z))
                    {
                        continue;
                    }
                }
    ...
    if (pixs > 0)
    {
        ...
        if (err)
        {
            fprintf(stderr,
                    ...
                    mc2str(g->mc), g->seed, g->dim, x, z, w, h, scale);
            for (int i = 0; i < w * h; i++)
                biomes[i] = -1;
        }
...
void Level::init4map(QWorld *w, int pix, int layerscale)
{
    ...
    if (dim == 0 && wi.mc >= MC_1_18)
    {
        ...
        {
            case LOPT_NOISE_T_4:
                g.bn.nptype = NP_TEMPERATURE;
                break;
            case LOPT_NOISE_H_4:
                g.bn.nptype = NP_HUMIDITY;
                break;
            case LOPT_NOISE_C_4:
                g.bn.nptype = NP_CONTINENTALNESS;
                break;
            case LOPT_NOISE_E_4:
                g.bn.nptype = NP_EROSION;
                break;
            case LOPT_NOISE_D_4:
                g.bn.nptype = NP_DEPTH;
                break;
            case LOPT_NOISE_W_4:
                g.bn.nptype = NP_WEIRDNESS;
                break;
        }
    }
}
...
void Level::resizeLevel(std::vector<Quad *> &cache, int x, int z, int w, int h)
{
    ...
    std::sort(togen.begin(), togen.end(), [](Quad *a, Quad *b) {
...
static bool draw_grid_rec(QPainter &painter, QRect &rec, qreal pix, int x, int z)
{
    ...
    QRect textrec = painter.fontMetrics().boundingRect(rec, Qt::AlignLeft | Qt::AlignTop, s);
...
void QWorld::draw(QPainter &painter, int vw, int vh, qreal focusx, qreal focusz, qreal blocks2pix)
{
    ...
    if (sshow[D_SLIME] && dim == 0 && blocks2pix * 16 > 0.5)
    {
        ...
        if (x < slimex || z < slimez || x + w >= slimex + slimeimg.width() || z + h >= slimez + slimeimg.height() ||
    ...
    {
        Level &l = lvs[sopt];
        //qDebug() << "render level " << sopt << "thisone?" << (sopt == D_REMOTE) << "cont: " << !sshow[sopt]
        //         << (dim != l.dim) << (l.vis > blocks2pix);
        ...
        for (Quad *q : l.cells)
        {
            ...
            for (VarPos &vp : *q->spos)
            {
                ...
                if (sopt == D_REMOTE)
                {
                    qDebug() << vp.p.x << vp.p.z << blocks2pix << x << y << (x < 0) << (x >= vw) << (y < 0)
                             << (y >= vh);
                }
                */
                ...
                */
                ...
                {  // don't draw selected structure
                    //qDebug() << "dont draw" << (selopt == sopt) << (selvp.p.x == spos.x) << (selvp.p.z == spos.z);
                    continue;
                }
                ...
                if (sopt == D_REMOTE)
                {
                    painter.drawText(QPointF(x - icon.width(), y - (icon.height()) / 2.F), "hi there!");
                }
            }
        }
        ...
        for (auto &it : frags)
        {
            //qDebug() << "benis " << sopt << *it.first;  //<< it.second;
    ...
    {  // draw selection overlay
        ...
        int pad = 5;
        QRect textrec = painter.fontMetrics().boundingRect(0, 0, vw, vh, Qt::AlignLeft | Qt::AlignTop, s);
        ...
        if (!sinfo.empty())
        {
            ...
            painter.fillRect(textrec.marginsAdded(QMargins(2, 0, 2, 0)),
                             QBrush(QColor(0, 0, 0, 128), Qt::SolidPattern));
            painter.drawText(textrec, s, QTextOption(Qt::AlignLeft | Qt::AlignVCenter));
        }
    }
